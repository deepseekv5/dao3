...
决定记在 `server/data/consent.json`，之后可在工作台左侧「模板授权」随时改。
**你已经改过的示例世界不会被模板覆盖**——只有没动过的程序化 demo 会被替换。

缺的部分（著作权原因不镜像，也不可能在包内）：

| 内容 | 状态 |
| --- | --- |
| 40 个官方音效 mp3 | ❌ 需自有权限取回：`npm run fetch:official`（按 CID 下载并校验 sha256） |
| 赛道模型 (.vb → glb) | ❌ 不在分发范围内，缺模型时实体以占位盒显示 |

要拿到**含真实模型资源**的完整工程，正确路径仍是官方编辑器里的
