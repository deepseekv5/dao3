...
校验内容一致（该实现已用官方 40 个音频 40/40 验证）。

**取回后能得到什么**（以下是在一份全新克隆上实测的结果，不是推断）：

| 内容 | 是否恢复 |
|---|---|
| 199 个实体（报名点 / 检查点 / 加速道具 / 场景模型）与其 tags、bounds、anchorOffset | ✅ |
| 官方 `player`(41 键) / `physics`(gravity −0.1, velocityDamping 0.01) / `environment` / `ambientSound` / `uiTree` | ✅ |
| 40 个官方音频 | ✅ |
| 竞速脚本 `index.js` | ⚠️ 只有当你把 `code-index.js` 放进 `official-project/racing-template/` 才导入——脚本不是 CID 可寻址的，清单里没有它 |
| **1,523,592 格 / 256×128×256 的体素地形** | ❌ 官方 `voxels` 指向的是烘焙好的网格 chunk（私有二进制，八叉树解码已证伪），无法还原成体素；种子世界地形是程序化生成的 48×32×48 |
| 赛道模型 (.vb → glb) | ❌ 不在分发范围内，缺模型时实体以占位盒显示 |

要把**完整**的官方地图（含真实地形与脚本）拿进来，正确路径是官方编辑器里的
「导出项目包 (.zip)」，再用本项目的导入——项目包里地形、脚本、实体、素材是齐的：

```
编辑器 → 文件菜单 → 导入项目包 (.zip)
```

`fetch:official` 的定位是"按哈希恢复结构化元数据"，不是"复刻整张地图"。
