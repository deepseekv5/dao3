...
其中方块 ID 表与图集上游是 Apache-2.0 开源仓库；地图数据、20 个赛道模型与 40 个音效
**由仓库所有者决定随包分发**，但这不构成任何许可授予——它们仍不在任何开源许可之下。

因此本仓库默认不放行这些内容：地图不会自动装进世界库，模型与音效的请求在服务端一律
`403`，直到你在应用内确认自己**有权获取并在本地使用**它们（仅用于学习与兼容性研究）。
仓库被裁剪或字节对不上时，可用你自有权限执行 `npm run fetch:official` 按 CID 取回，
脚本会逐文件校验 CIDv0（`base58(0x12 0x20 ‖ sha256)`）再落盘；取回不等于放行，
那道确认仍然生效。
