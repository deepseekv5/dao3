...
```bash
npm run build:play        # → play/（82 个文件，约 8.4 MB）
npm run test:play         # 真浏览器回归：桌面 + 手机 + CDN 解压，38 条断言
```
...
音频与模型目录整块禁掉，运行时必需的文件逐个断言在场（含 GLTFLoader、BufferGeometryUtils 与素材包代表文件）。

## 模型与音效：随包，确认点挪到首屏那张卡

官方音效与赛道模型（20 个 `.gltf`）著作权归 box3lab，**不在任何开源许可之下**；
它们由仓库所有者决定随包分发，桌面版靠服务端那道 `403` 闸门兜住。
静态托管没有服务端可点头，所以确认点改到首屏：那张卡列明来源、著作权，
以及"点「进入体验」即表示你确认自己有权获取并使用它们"。这是同一决定的等价实现，不是绕过。

`build-play.mjs` 把 `official-project/racing-assets/` 整目录复制进 `play/assets/`，
缺目录就直接失败——体验版不该在没模型没音效的情况下发布。

摆放走的是编辑器同一条路（`main.js` 的 `applySceneModels`）：把带 mesh 的实体克隆成
**外部对象**放进 `state.models`，`_buildRegistry` 才会带上 `orientation` 与 `scaleVec`。
一开始图省事只把网格交给运行时的 `_setMesh`，结果 199 个模型全按默认朝向堆在原点，
玩家一出生就被埋进一片青色围栏——截图整屏纯色，断言却全绿。
`play.js` 里那段注释就是为下一个想走捷径的人写的。

回归对应三条断言：控制台必须出现 `index.js 运行成功` 与 `clientIndex.js 运行成功`、
**不得**出现 `占位|未找到资产`、且实体必须真的挂上网格
（`_meshHolder.children.length > 0` 的超过 150 个）。第三条专门堵
"往 `assets.meshes` 塞空 Group 让告警消失"这种自欺做法——那样画面是一片空白，
而前两条照样全绿。
